...
# Определение аугментаций с сниженными порогами
transform = A.Compose([
    A.HorizontalFlip(p=0.5),  # Переворот по горизонтали
    A.RandomBrightnessContrast(p=0.3, brightness_limit=0.1, contrast_limit=0.1),  # Уменьшены пределы
    #A.GaussNoise(p=0.3, var_limit=(1.0, 5.0)),  # Снижен шум
    A.ShiftScaleRotate(p=0.3, shift_limit=0.05, scale_limit=0.05, rotate_limit=10),  # Уменьшены сдвиг и поворот
    A.Blur(p=0.2, blur_limit=2),  # Снижен предел размытия
    A.CoarseDropout(p=0.2, max_holes=1, max_height=10, max_width=10),  # Уменьшены размеры dropout
    A.ColorJitter(p=0.3, brightness=0.1, contrast=0.1, saturation=0.1, hue=0.05),  # Уменьшены изменения цвета
